...
-- this gives a crappy average of the length of each word that throws away the remainer.

-- 3. We’d really like the answer to be more precise. Can you rewrite that using fractional division?

seekritFuncDouble x = (/) (fromIntegral (sum (map length (words x))))
                          (fromIntegral (length (words x)))

-- Rewriting functions using folds

-- In the previous chapter, you wrote these functions using direct recur- sion over lists. The goal now is to rewrite them using folds. Where possible, to gain a deeper understanding of folding, try rewriting the fold version so that it is point-free.
...
myMaximumBy f (x:xs) = foldr (\x y -> case f x y of
                                        LT -> y
                                        _ -> x) x xs
-- 11. myMinimumBy takes a comparison function and a list and returns the least element of the list based on the last value that the com- parison returned LT for.
