...
-- these operators mean the brackets will be parsed and then discarded but the p
-- will remain as our result
...
parseAssignment = do name <- some letter
                     _ <- char '='
                     val <- some (noneOf "\n")
                     skipEOL -- important!
                     return (name, val)
...
parseSection = do skipWhitespace
                  skipComments
                  h <- parseHeader
                  skipEOL
...
main = hspec $ do
  ...
  describe "Comment parsing" $ it "Can skip a comment before a header" $ do
